...
-------Configs-------
local RadioVolume = 0.4
local micclick = true
local micClickVolume = 0.1
local micClickOn = true
local micClickOff = true
local speakerRange = 1.0 -- listen voice when player in Radio/call 
local callSpeakerEnabled = true
local radioEnabled = true
local radioClickMaxChannel = 288
--========================================================
--                    FUNCTIONS
--========================================================
function SetVoiceData(key, value, target)
	TriggerServerEvent("cc-voice:SetVoiceData", key, value, target)
end

function PlayMicClick(channel, value)
	if channel <= radioClickMaxChannel then
		if micclick then
			if (value and micClickOn) or (not value and micClickOff) then
				SendNUIMessage({ sound = (value and "audio_on" or "audio_off"), volume = micClickVolume })
			end
		end
	end
end
...
function SetGridTargets(pos, reset) -- Used to set the players voice targets depending on where they are in the map
	...
	if reset then
		NetworkSetTalkerProximity(Config.voiceModes[playerData.mode][1] + 0.0) -- Set voice proximity
...
AddEventHandler("onClientResourceStart", function(resName) -- Initialises the script, sets up voice range, voice targets and request sync with server
	...
	if Config.useExternalServer then
		MumbleSetServerAddress(Config.externalAddress, Config.externalPort)
	end
	...
	TriggerServerEvent("cc-voice:Initialise")

	SendNUIMessage({ speakerOption = callSpeakerEnabled })

	TriggerEvent("cc-voice:Initialised")

	initialised = true
end)

RegisterNetEvent("cc-voice:SetVoiceData") -- Used to sync players data each time something changes
AddEventHandler("cc-voice:SetVoiceData", function(player, key, value)
...
RegisterNetEvent("cc-voice:SyncVoiceData") -- Used to sync players data on initialising
AddEventHandler("cc-voice:SyncVoiceData", function(voice, radio, call)
	voiceData = voice
	radioData = radio
	callData = call
end)

RegisterNetEvent("cc-voice:RemoveVoiceData") -- Used to remove redundant data when a player disconnects
AddEventHandler("cc-voice:RemoveVoiceData", function(player)
...
--                   Commands
--========================================================
RegisterCommand('radio_volume', function(source, args)
    local vol = tonumber(args[1])
    if vol then
		RadioVolume = vol / 100
		TriggerEvent('notification','Volume: '..vol..' /100',1)
	else
		TriggerEvent('notification','Invalid Argument',2)
    end
end)

RegisterCommand('radio_click',function(channel, value)
	click = not click
	if click then
		micclick = true
		TriggerEvent('notification','Radio Chatter on',1)
	else
		micclick = false
		TriggerEvent('notification','Radio Chatter off',1)
	end
end)
--========================================================
--                  Citizen Threads
	...
	while true do
		...
		if initialised then
			...
			if IsControlJustPressed(0, Config.controls.proximity.key) then
				...
				if Config.controls.speaker.key ~= Config.controls.proximity.key or Config.controls.speaker.secondary == nil then
					secondaryPressed = false
				else
					secondaryPressed = IsControlPressed(0, Config.controls.speaker.secondary) and (playerData.call > 0)
				end
				...
				if not secondaryPressed then
					...
					NetworkSetTalkerProximity(Config.voiceModes[voiceMode][1] + 0.0)
			...
			if callSpeakerEnabled then
				local secondaryPressed = false

				if Config.controls.speaker.secondary ~= nil then
					secondaryPressed = IsControlPressed(0, Config.controls.speaker.secondary)
				else
					secondaryPressed = true
				end

				if IsControlJustPressed(0, Config.controls.speaker.key) and secondaryPressed then
	...
	while true do
		if initialised then
			...
			SendNUIMessage({
				...
				mode = Config.voiceModes[playerMode][2],
				radio = Config.radioChannelNames[playerRadio] ~= nil and Config.radioChannelNames[playerRadio] or playerRadio,
				radioActive = playerRadioActive,
				call = Config.callChannelNames[playerCall] ~= nil and Config.callChannelNames[playerCall] or playerCall,
...
Citizen.CreateThread(function()
	while true do
		if initialised then
			if Config.use2dAudioInVehicles then
				local playerPed = PlayerPedId()
				local playerData = voiceData[playerServerId]
