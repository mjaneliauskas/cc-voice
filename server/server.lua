...
AddEventHandler("onResourceStart", function(resName) -- Initialises the script, sets up voice related convars
	...
	SetConvarReplicated("voice_useNativeAudio", Config.useNativeAudio and "true" or "false")
	SetConvarReplicated("voice_use2dAudio", Config.use3dAudio and "false" or "true")
	SetConvarReplicated("voice_use3dAudio", Config.use3dAudio and "true" or "false")	
	SetConvarReplicated("voice_useSendingRangeOnly", Config.useSendingRangeOnly and "true" or "false")	
...
RegisterNetEvent("cc-voice:Initialise")
AddEventHandler("cc-voice:Initialise", function()
	...
	TriggerClientEvent("cc-voice:SyncVoiceData", -1, voiceData, radioData, callData)
end)

RegisterNetEvent("cc-voice:SetVoiceData")
AddEventHandler("cc-voice:SetVoiceData", function(key, value, target)
	if not voiceData[source] then
		voiceData[source] = {
			mode = 2,
			radio = 0,
	...
	if key == "speakerTargets" then
		TriggerClientEvent("cc-voice:SetVoiceData", -1, target, key, value)
	else
		TriggerClientEvent("cc-voice:SetVoiceData", -1, source, key, value)
	end
end)
...
AddEventHandler("playerDropped", function()
	if voiceData[source] then
		...
		TriggerClientEvent("cc-voice:RemoveVoiceData", -1, source)
	end
end)
...
function SetPlayerRadioName(serverId, name)
	if voiceData[serverId] then
		...
		TriggerClientEvent("cc-voice:SetVoiceData", -1, serverId, "radioName", value)
	end
end
--========================================================
--                  EXPORTS
